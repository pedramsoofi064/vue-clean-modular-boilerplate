...
// node_modules/vue-imask/node_modules/vue-demi/lib/index.mjs
var isVue3 = true;
var Vue2 = void 0;
...
function useIMask(props2, _temp) {
  let {
    emit,
    onAccept,
    onComplete,
  ...
  const _props = isRef(props2) ? props2 : ref(props2);
  const el = ref();
  const mask = ref();
  const masked = ref("");
  const unmasked = ref("");
  ...
  const updateUnmaskedValue = () => {
    if (!mask.value || unmasked.value === void 0)
      return;
    if ($lastAcceptedUnmaskedValue !== unmasked.value) {
      mask.value.unmaskedValue = unmasked.value;
      if (mask.value.unmaskedValue !== unmasked.value)
        _onAccept();
    }
    $lastAcceptedUnmaskedValue = void 0;
  };
  watch(unmasked, updateUnmaskedValue);
  const updateMaskedValue = () => {
    if (!mask.value || masked.value === void 0)
      return;
    if ($lastAcceptedValue !== masked.value) {
      mask.value.value = masked.value;
      if (mask.value.value !== masked.value)
        _onAccept();
    }
    $lastAcceptedValue = void 0;
  };
  watch(masked, updateMaskedValue);
  const updateTypedValue = () => {
    if (!mask.value || typed.value === void 0)
      return;
    if ($lastAcceptedTypedValue !== typed.value) {
      mask.value.typedValue = typed.value;
      if (!mask.value.masked.typedValueEquals(typed.value))
        _onAccept();
    }
    $lastAcceptedTypedValue = void 0;
  };
  watch(typed, updateTypedValue);
  function _initMask() {
    $el = el.value;
    const $props = _props.value;
    if (!$el || !($props != null && $props.mask))
      return;
    mask.value = IMask($el, $props);
    if (defaultValue !== void 0)
      masked.value = defaultValue;
    if (defaultUnmaskedValue !== void 0)
      unmasked.value = defaultUnmaskedValue;
    if (defaultTypedValue !== void 0)
      typed.value = defaultTypedValue;
    updateUnmaskedValue();
    updateMaskedValue();
    updateTypedValue();
    storeLastAcceptedValues();
  ...
  watch([el, _props], () => {
    ...
    const $props = _props.value;
    if (!($props != null && $props.mask) || $newEl !== $el)
      _destroyMask();
    if ($newEl) {
      if (!mask.value) {
        _initMask();
...
function extractOptionsFromProps(props2, exclude) {
  props2 = {
    ...props2
  };
  Object.keys(props2).forEach((prop) => {
    if (props2[prop] === void 0 || exclude.includes(prop))
      delete props2[prop];
  });
  return props2;
}
...
var Component3 = defineComponent({
  ...
  setup(props2, _ref) {
    ...
    const {
      ...
      unmasked,
      typed
    } = useIMask(extractOptionsFromProps(props2, VALUE_PROPS), {
      emit,
      onAccept: (event) => {
    ...
    const pvalue = toRef(props2, "value");
    const pmodelValue = toRef(props2, "modelValue");
    const punmasked = toRef(props2, "unmasked");
    const ptyped = toRef(props2, "typed");
    ...
    return () => {
      const data = {
        ...attrs,
        value: props2.value != null ? props2.value : props2.modelValue != null ? props2.modelValue : mask.value ? mask.value.displayValue : "",
        ref: el
      };
      if (!props2.mask) {
        data.onInput = (event) => {
  ...
  methods: {
    _maskValue() {
      ...
      if (this.unmask === "typed")
        return (_this$maskRef = this.maskRef) == null ? void 0 : _this$maskRef.typedValue;
      if (this.unmask)
        return (_this$maskRef2 = this.maskRef) == null ? void 0 : _this$maskRef2.unmaskedValue;
